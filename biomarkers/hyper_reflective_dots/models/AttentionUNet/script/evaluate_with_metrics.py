...
from model.attention_unet_model import AttentionUNet

# ===== CONFIG =====
IMAGES_DIR = "/content/test_324/test/images"
MASKS_DIR = "/content/test_324/test/masks"
MODEL_PATH = "/content/outputs/checkpoints/best_model.pth"
...
# Load custom AttentionUNet model
model = AttentionUNet(in_channels=1, out_classes=1).to(DEVICE)
model.load_state_dict(torch.load(MODEL_PATH, map_location=DEVICE, weights_only=True))
model.eval()

print(f"Loaded custom AttentionUNet from: {MODEL_PATH}")

# ===== BATCH PROCESSING =====
image_files = sorted([f for f in os.listdir(IMAGES_DIR) if f.endswith(('.png', '.jpg', '.jpeg', '.tif'))])
all_dice, all_iou = [], []
results_to_plot = []
...
with torch.no_grad():
    for i, filename in enumerate(tqdm(image_files)):
        ...
        mask_tensor = aug["mask"].unsqueeze(0).to(DEVICE)  # Add batch dim for consistency
...
# ===== FINAL STATS =====
print(f"\n{'='*50}")
print(f"Evaluation Results ({len(all_dice)} images)")
print(f"{'='*50}")
